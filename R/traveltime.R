#' @export
traveltime <- function(facilities, bb_area, dowscaling_model_type, mode){
...
handle <- curl::new_handle(timeout = 120)
  ...
  r <- raster::raster(raster::extent(bb_area %>% st_transform(3395)), res = 10, crs=st_crs(3395)$proj4string)

  streets <- fasterize::fasterize(st_buffer(st_transform(x$osm_lines, 3395), 10), r, background=NA)
  ...
  if(mode !="walk"){
    ...
    # for walking
    friction_dowscaled <- d[[1]]
    raster::values(friction_dowscaled) <- 0.012
  ...
  points = as.data.frame(sf::st_coordinates(cooling))
