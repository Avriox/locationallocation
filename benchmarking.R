...
run_benchmark <- function(i) {
  ...
  alloc_result_discrete_time <- system.time(alloc_result_discrete <- allocation_discrete(demand_raster = pop, bb_area = boundary, facilities=fountains, candidate=candidates, n_fac = params$n_fac[i], weights=NULL, objectiveminutes=15, dowscaling_model_type="lm", mode="walk", res_output=params$res_output[i], n_samples=params$n_samples[i], par=F))
